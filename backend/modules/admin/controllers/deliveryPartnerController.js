...
export const createDeliveryPartner = asyncHandler(async (req, res) => {
  try {
    ...
    // Prepare location data from zones (simplified for now)
    // You might want to enhance this to map zones to actual location data
    const location = {
      addressLine1: '',
      city: zone || '', // Using zone as city for now as fallback
      state: '',
      country: 'India'
    };
    ...
    const delivery = new Delivery({
      ...
      status: 'approved', // Auto-approve since created by admin
      isActive: true,
      verifiedAt: new Date(),
      ...
      location,
      vehicle: {
        type: vehicle ? vehicle.toLowerCase() : 'bike',
        number: '', // Can be updated later
        model: '',
        brand: ''
      },
      documents: {
        [identityType === 'Driving License' ? 'drivingLicense' : 'aadhar']: { // Simplified mapping
          number: identityNumber,
          document: identityImage,
          verified: true
        }
      },
      availability: {
        zones: [] // You might want to loop up zone ID here if you have it
      },
      salary: {
        type: 'fixed',
    ...
    // Create wallet for the new delivery partner
    const DeliveryWallet = (await import('../../delivery/models/DeliveryWallet.js')).default;
    await DeliveryWallet.findOrCreateByDeliveryId(delivery._id);

    logger.info(`Delivery partner created by admin: ${delivery._id}`, {
      adminId: req.user._id,
      deliveryId: delivery.deliveryId
    });

    return successResponse(res, 201, 'Delivery partner created successfully', {
      delivery: {
        _id: delivery._id.toString(),
        name: delivery.name,
    ...
    logger.error(`Error creating delivery partner: ${error.message}`, { error: error.stack });
    return errorResponse(res, 500, 'Failed to create delivery partner');
  }
});
...
export const updateDeliveryPartner = asyncHandler(async (req, res) => {
  try {
    const { id } = req.params;
    const { name, email, phone, vehicle, salary, joiningDate } = req.body;
    ...
    if (vehicle) {
      if (vehicle.type) delivery.vehicle.type = vehicle.type;
      if (vehicle.number) delivery.vehicle.number = vehicle.number;
      if (vehicle.model) delivery.vehicle.model = vehicle.model;
      if (vehicle.brand) delivery.vehicle.brand = vehicle.brand;
    }

    // Update Salary Config
    if (salary) {
      // Initialize salary object if it doesn't exist (for migration)
      if (!delivery.salary) {
        delivery.salary = { type: 'commission', amount: 0 };
      }

      if (salary.type) delivery.salary.type = salary.type;
      if (salary.amount !== undefined) delivery.salary.amount = Number(salary.amount);
    }

    if (joiningDate) delivery.joiningDate = joiningDate;

    await delivery.save();

    logger.info(`Delivery partner updated: ${id}`, {
      updatedBy: req.user._id
    });

    return successResponse(res, 200, 'Delivery partner updated successfully', {
      delivery: {
        _id: delivery._id.toString(),
        name: delivery.name,
    ...
    logger.error(`Error updating delivery partner: ${error.message}`, { error: error.stack });
    return errorResponse(res, 500, 'Failed to update delivery partner');
  }
});
